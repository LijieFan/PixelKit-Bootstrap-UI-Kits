...
function nextbus_html(times) {
    ...
    times.forEach(function (d, i) {
        if (i === times.length - 1) {
            html += "<strong>" + d + "&nbsp min" + "</strong>" ;
        } else {
            html += "<strong>" + d + " &nbsp| &nbsp" + "</strong>" ;
        }
    });
...
function setupHubway() {
    $.getJSON('data/hubway.json', function(data) {
        ...
        $("#hubway .title").html("<strong>BlueBikes@SP (updated " +
                               data['time_updated'] + ")</strong>");
        $("#hubway .content").html("<strong>" + data['num_bikes_available'] + " Bikes | " +
